	...
	for animation_name in animation_names:
		...
		if p_animation_conversion_table.has(animation_name):
			...
			for track_idx in range(0, animation.get_track_count()):
				...
				############################
				if animation.track_get_type(track_idx) == Animation.TYPE_TRANSFORM3D:
			...
			for skeleton in p_skeletons:
				...
				if track_idx == -1:
					track_idx = animation.add_track(Animation.TYPE_TRANSFORM3D)
				...
				for skeleton in skeletons:
					for i in range(0, skeleton.get_bone_count()):
						...
						if animation.find_track(path) == -1:
							var track_idx: int = animation.add_track(Animation.TYPE_TRANSFORM3D)
							animation.track_set_path(track_idx, path)
							animation.track_insert_key(track_idx, 0.0, convert_tranform_to_transform_value(Transform3D()))
							animation.track_insert_key(track_idx, animation.length, convert_tranform_to_transform_value(Transform3D()))
							animation.track_set_imported(track_idx, true)
				# Save the animation again if it was saved externally
				if !animation.resource_local_to_scene:
					ResourceSaver.save(animation.resource_path, animation)
